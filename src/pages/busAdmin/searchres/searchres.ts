...
import { BusBaseInfoBean } from '../../../beans/beans';
...
@Component({
  templateUrl: 'searchres.html'
})
export class BusAdminSearchResult implements OnInit, OnDestroy {
  ...
  public mapControler: BusAdminSearchMap;
  ...
  public busList: BusBaseInfoBean[] = [];
  ...
  doInfinite(ev) {
    this.busAdminService.getBusesByDepartId(this.departMentId, 0, 20).subscribe(itemList => {
      if (itemList) {
        this.busList = this.busList.concat(itemList);
      }
    });
  }
  ...
  onClickItem(bus) {
    this.busAdminService.getCurrentDirverAndBusInfoByBusNo(bus.busno).subscribe(info => {
      if (info) {
        this.navCtrl.push(BusAdminBusDetail, { baseinfo: info });
      }
    });
  }
}
